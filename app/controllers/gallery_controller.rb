class GalleryController < ApplicationController
  	...
  	def checkout
		...
		if request.post?
			...
			if response.success?
				...
				#flash[:notice]="Thank You for using our Watch. The order details are sent to your mail"          
				session[:cart_id]=nil
				session[:amount]=nil
  	...
  	def search
    	@image_gallery = Store.find_by_sql ["Select * from stores WHERE name like ? or category like ? or color like ? or description like ?",params[:search],params[:search],params[:search],params[:search]]	  
  	end
